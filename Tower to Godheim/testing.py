...
def damage_calculation():
    ...
    dmg_block_player = enemy.atk * player.armour_value
...
def player_crit():
    ...
    enemy.hp = enemy.hp - player.atk
    print(player.name, "crit and deal", round(player.atk) , "damage to", enemy.name, "(", round(dmg_block_enemy), "damage blocked)", "\n")
    player.turn = 0
...
def player_status():
	...
	if player.main_hand.effect == "bleed":
		...
		print(player.name, "crit and deal", round(player.atk) , "damage to", enemy.name, "(", round(dmg_block_enemy), "damage blocked)", "and apply", player.main_hand.effect, "\n")
...
def enemy_hit():
    ...
    print(enemy.name, "deal", round(enemy.atk), "damage to", player.name, "(", round(dmg_block_player), "damage blocked)", "\n")
    player.turn = 1

def enemy_crit():
    enemy.atk = enemy.atk * 2
    player.hp = player.hp - enemy.atk
    print(enemy.name, "crit and deal", round(enemy.atk) , "damage to", player.name, "(", round(dmg_block_player), "damage blocked)", "\n")
    player.turn = 1
...
def enemy_status():
	...
	if enemy.main_hand.effect == "bleed":
		...
		print(enemy.name, "crit and deal", round(enemy.atk) , "damage to", player.name, "(", round(dmg_block_player), "damage blocked)", "and apply", enemy.main_hand.effect)
...
damage_calculation()
combat()
print (player.armour_chest.stat_str)